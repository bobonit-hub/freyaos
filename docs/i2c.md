...
one of those at a time: opening I2C while PWM still drives it, or the other
way round, returns `FREYA_ERR_BUSY`.
...
| | |
|---|---|
| `FREYA_ERR_BUSY` | the bus is open from the other side, or a pin is already PWM |
